...
initialAge = np.array([params['movement.initialAge.%s' %m][0] for m in map_index]).astype(int)
lastAge = np.array([params['movement.lastAge.%s' %m][0] for m in map_index]).astype(int)
np.unique(initialAge), np.unique(lastAge)
