 ...
 * @author pverley
 */
public class BackgroundMapDistribution extends OsmoseLinker {
    ...
    public void init() {
        ...
        maps = new BackgroundMapSet(iSpeciesFile, iSpecies, "movement");
        try {
            maps.init();
        } catch (IOException | InvalidRangeException ex) {
            Logger.getLogger(BackgroundMapDistribution.class.getName()).log(Level.SEVERE, null, ex);
        }
    ...
    public void move(BackgroundSchool school, int iStepSimu) {
        this.mapsDistribution(school, iStepSimu);
    }
    ...
    public boolean mapChange(int iStepSimu, int classIndex) {
        ...
        if (iStepSimu > 0) {
            int previousIndexMap = maps.getIndexMap(classIndex, iStepSimu - 1);
     ...
     * @return
     */
    private List<Cell> getAccessibleCells(BackgroundSchool school, GridMap map) {
