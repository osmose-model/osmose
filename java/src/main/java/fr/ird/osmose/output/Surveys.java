...
public class Surveys extends AbstractOutputRegion {
    ...
    public void init() {
        ...
        this.setName(getConfiguration().getString("surveys.name.sr" + index).replaceAll("_", "").replaceAll("-", ""));
        if (!this.getName().matches("^[a-zA-Z0-9]*$")) {
            error("Survey name must contain alphanumeric characters only. Please rename " + this.getName(), null);
        }
