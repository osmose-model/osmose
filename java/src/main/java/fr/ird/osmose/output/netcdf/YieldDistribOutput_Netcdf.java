...
public class YieldDistribOutput_Netcdf extends AbstractDistribOutput_Netcdf {
    ...
    public YieldDistribOutput_Netcdf(int rank, OutputDistribution distrib) {
        super(rank, distrib);
    }
    ...
    public void update() {
        for (School school : getSchoolSet().getAliveSchools()) {
            ...
            if (classSchool >= 0) {
                values[school.getFileSpeciesIndex()][getClass(school)] += school.abd2biomDeadSchools(school.getNdead(MortalityCause.FISHING));
            }
        }
    }
