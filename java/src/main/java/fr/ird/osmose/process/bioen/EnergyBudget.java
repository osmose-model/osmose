...
public class EnergyBudget extends AbstractProcess {
    ...
    public void run() {
        ...
        for (School school : getSchoolSet().getAliveSchools()) {
			...
			if (school.isOut()) {
				...
				try {
					school.updateBioenMaturation();   // computes maturation properties for the species.
				} catch (Exception ex) {
    ...
    public void updateEgross(School school) {
        int ispec = school.getSpeciesIndex();
        double output = school.getIngestion() * this.assimilation[ispec] * temp_function.getPhiT(school) * oxygen_function.getFO2(school);
