...
public class ForagingMortality extends AbstractMortality {
    ...
    public double getRate(School school) {
        ...
        if (this.getConfiguration().isGeneticEnabled()) {
            ...
            double Imax_temp = I_max[school.getSpeciesIndex()];

            try {
                Imax_temp = ((School) school).existsTrait(key) ? ((School) school).getTrait(key)
                        : I_max[school.getSpeciesIndex()];
            } catch (Exception e) {
            ...
            try {
                // output = school.getTrait(key) * this.k_for[school.getSpeciesIndex()] /
                // nstepYear;
                output = this.k1_for[school.getSpeciesIndex()]
                        * Math.exp(
                                this.k2_for[school.getSpeciesIndex()] * (Imax_temp - I_max[school.getSpeciesIndex()]))
                        / nstepYear;
            } catch (Exception ex) {
