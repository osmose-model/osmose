...
fpath = "submodel/_static/plot_random_dis.py"
...
: Random distribution parameter {#tbl-random-mov .striped .hover tbl-colwidths="[60, 40]"}
...
- Else, one cell is first randomly picked up in the whole domain, and the domain is extended from neighbours to neighbours until the number of cells reaches `ncell` (cf. @fig-random-dis)

![Initialization process of species domain when random distribution is used with a limited number of cells.
Black squares indicate land cells, colors and numbers indicate the iteration step when the cell is included in the domain.](submodel/_static/random_drift.png){#fig-random-dis fig-align="center"}
...
A single map distribution can be defined either using CSV (@tbl-csvmap-mov) or NetCDF (@tbl-ncmap-mov) files
...
: CSV map distribution parameter {#tbl-csvmap-mov .striped .hover tbl-colwidths="[50, 40]"}
...
: NetCDF map distribution parameter {#tbl-ncmap-mov .striped .hover tbl-colwidths="[50, 40]"}
