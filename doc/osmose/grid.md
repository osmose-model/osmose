...
```{index} single: grid.ncolumn single: grid.nline single: grid.lowright.lat single: grid.lowright.lon single: grid.upleft.lat single: grid.upleft.lon single: grid.mask.file
```
...
Furthermore, an additional optional parameter has been added, {samp}`grid.var.surf`, which provides the name of the cell surface variable (which must be defined in $m^2$).
