...
$M(2, p(t'))$ for 2 trials with a probability vector $p(t')$ composed of as many elements $p_i(t')$ as there are schools in the population. The $i^{th}$ element of
$p_i(t')$ is defined as the relative fecundity of school $i$  in the population at the initial time step of the breeding season $t$:
...
with $N_{eggs}(i, t)$ the fecundity of school $i$ and  $\sum_{j \lor s(j) = s(i)} N_{eggs(j, t')}$ the total fecundity
of the species $s(i)$ population at the initial time step of the breeding season $t$.
