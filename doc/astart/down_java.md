...
{samp}`JRE`: Java Runtime Environment. It is a kit distributed by Sun to execute Java programs. A {samp}`JRE` provides a {samp}`JVM` and some basic Java libraries. **A JRE is needed to run Osmose**. It can be downloaded from [https://www.java.com/fr/download/](https://www.java.com/fr/download/).
