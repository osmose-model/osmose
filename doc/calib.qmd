...
In this section, the methodology of {{ os }} calibration using the sequential approach described in {cite}`Oliveros2017` is provided. This calibration uses the `calibrar` and `osmose` R libraries (see {numref}`rlibs` for installation instructions). For a detailed description of the `calibrar` package, the user is referred to {cite}`Oliveros2017bis`.
