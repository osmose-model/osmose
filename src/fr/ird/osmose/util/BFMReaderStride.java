 ...
 * @author pverley
 */
public class BFMReaderStride extends JPanel {
    ...
    private String ncpathname = "/run/media/pverley/phynx/storage/bfm/adriatic_emanuela-clementi/result_bfm199001_2w.nc";
    private float[][] variable;
    private float[][] variablestr;
    ...
    private float[][] lonstr, latstr, maskstr;
    ...
    private void makeStriddenVariable() {
        imstr = im / stride;
        jmstr = jm / stride;
        lonstr = new float[jmstr][imstr];
        latstr = new float[jmstr][imstr];
        maskstr = new float[jmstr][imstr];
        variablestr = new float[jmstr][imstr];
                ...
                for (int ii = 0; ii < stride; ii++) {
                    for (int jj = 0; jj < stride; jj++) {
                        ...
                        for (int t = 0; t < variable.length; t++) {
                            float valtmp = getValue(i * stride + ii, j * stride + jj, 0, t);
                            if (!Float.isNaN(valtmp)) {
                                value += getValue(i * stride + ii, j * stride + jj, 0, t);
                                counter++;
                            }
                        }
                    }
    ...
    private float findMaxValue() {
        ...
        for (int j = 0; j < jmstr; j++) {
            for (int i = 0; i < imstr; i++) {
                float tmp = computeMeanValueSurface(i, j);
                if (tmp > max) {
                    max = tmp;
                }
            }
    ...
    private float getValue(int i, int j, int k, int time) {
    ...
    private float computeMeanValueSurface(int i, int j) {
    ...
    public static void main(String args[]) {
        ...
        BFMReaderStride bfm = new BFMReaderStride();
        bfm.init();
        bfm.setGridVisible(true);

        //1. Create the frame.
        JFrame frame = new JFrame("BFM stridden grid - " + varname + " variable at sea surface.");
