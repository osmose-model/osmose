...
Jar files will be automatically downloaded in a temporary directory. However in order to set its location you will need to set up the `OSMOSE_DIR` environment variable. This is done by editing the `.Renviron` file, as shown [here](https://cran.r-project.org/web/packages/startup/vignettes/startup-intro.html).
